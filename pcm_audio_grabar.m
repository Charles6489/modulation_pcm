function varargout = pcm_audio_grabar(varargin)
% PCM_AUDIO_GRABAR MATLAB code for pcm_audio_grabar.fig
%      PCM_AUDIO_GRABAR, by itself, creates a new PCM_AUDIO_GRABAR or raises the existing
%      singleton*.
%
%      H = PCM_AUDIO_GRABAR returns the handle to a new PCM_AUDIO_GRABAR or the handle to
%      the existing singleton*.
%
%      PCM_AUDIO_GRABAR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PCM_AUDIO_GRABAR.M with the given input arguments.
%
%      PCM_AUDIO_GRABAR('Property','Value',...) creates a new PCM_AUDIO_GRABAR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before pcm_audio_grabar_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to pcm_audio_grabar_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help pcm_audio_grabar

% Last Modified by GUIDE v2.5 17-Apr-2021 09:04:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @pcm_audio_grabar_OpeningFcn, ...
                   'gui_OutputFcn',  @pcm_audio_grabar_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before pcm_audio_grabar is made visible.
function pcm_audio_grabar_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to pcm_audio_grabar (see VARARGIN)
movegui('center');
set(handles.axes1,'Visible','off');
set(handles.axes2,'Visible','off');
% Choose default command line output for pcm_audio_grabar
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes pcm_audio_grabar wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = pcm_audio_grabar_OutputFcn(hObject, eventdata, handles) 
...
[input_sig orgnal_sampl_freq]=audioread('voz.wav');
nbits = audioinfo('voz.wav')
...
tiempograb = str2num(get(handles.edit5, 'String'))
 %Establecer variables globales
 %frecuencia de muestreo
 %Tiempo de grabacion
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
v1 = audiorecorder(fs, 16, 1);
...
senal_grabada=getaudiodata(v1, 'single');
audiowrite('voz.wav',senal_grabada,fs);
